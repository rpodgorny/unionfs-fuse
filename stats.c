...
void stats_init() {
	...
	stats_read_b = stats_read_k = stats_read_m = stats_read_g = 0;
	stats_written_b = stats_written_k = stats_written_m = stats_written_g = 0;
...
void stats_sprint(char *s) {
	...
	sprintf(s+strlen(s), "Bytes read: %u,%03u,%03u,%03u\n", stats_read_g, stats_read_m, stats_read_k, stats_read_b);
	sprintf(s+strlen(s), "Bytes written: %u,%03u,%03u,%03u\n", stats_written_g, stats_written_m, stats_written_k, stats_written_b);
}

void stats_add_read(long bytes) {
	stats_read_b += bytes;
...
void stats_add_written(long bytes) {
